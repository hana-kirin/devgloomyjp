<?php
/**
 * Template Name: トップLP（全画面）
 *
 * 使い方:
 * 1) このファイルを、有効なテーマのフォルダ（例: /wp-content/themes/お使いのテーマ/）にアップロード
 * 2) WordPress管理画面 → 固定ページ → 新規追加 でページを作成（例: タイトル「トップ」）
 *    右側の「テンプレート」で「トップLP（全画面）」を選択して公開
 * 3) 設定 → 表示設定 →「ホームページの表示」を「固定ページ」にして、ホームページに上のページを指定
 *
 * ※ テーマのヘッダー/フッターを読み込まず、完全に独立したフルスクリーンLPとして表示します。
 * ※ 波の動画を使う場合は wave.mp4 をサイト直下（ドメイン直下）にアップロードすればそのまま再生されます。
 */
?>
<!DOCTYPE html>
<html lang="ja">
...
<meta name="description" content="馬のいななき オフィシャルサイト">
<style>
:root{
...
.hero video{animation:crashIn 2.6s cubic-bezier(.16,.8,.24,1) both, drift 14s ease-in-out 2.6s infinite}
...
.hero .poster{z-index:0;background:
  radial-gradient(120% 80% at 70% 12%, rgba(120,120,120,.35), transparent 55%),
  radial-gradient(90% 70% at 20% 90%, rgba(255,255,255,.06), transparent 60%),
  linear-gradient(160deg,#1c1c1c 0%,#0c0c0c 55%,#050505 100%)}
...
<header class="hero" id="hero">
  ...
  <!-- 波の動画。wave.mp4 をサイト直下（ドメイン直下）に置くと自動再生されます -->
